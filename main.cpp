	...
	while (Novice::ProcessMessage() == 0) {
		...
		if(player.life > 0)
		{
			...
			if(isClear == 0)
			{
				...
				Novice::DrawBox(int(player.pos.x - (player.radius + 3)), int(player.pos.y - 32), (player.radius + 3) * 2, 6, 0.0f, 0xFFFFFFFF, kFillModeSolid);

				Novice::DrawBox(int(player.pos.x - (player.radius + 2)), int(player.pos.y - 31), int((player.radius + 2) * 2 * player.flyEnergy / player.maxFlyEnergy), 4, 0.0f, 0xFF0000FF, kFillModeSolid);
			}
		}
		else
		{
